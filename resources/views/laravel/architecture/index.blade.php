...
<div class="themes">
    ...
    <div class="theme">
        ...
        <p class="theme__text">
            Архитектуру Laravel нужно знать, если вы хотите писать приложения, которые будут расширяться кастомным кодом. <br><br>

            1) Реализовываем задуманный функционал <br>
            К примеру, создадим класс, который будет содержать метод проверяющий дату на валидность. <br>
            Создадим файл App\Services\DataCheck.php. В этом файле будет реализован класс DataCheck. <br>
            Только не забудьте указать пространство имен, которое указывает то, где находится файл. В данном случае App\Services.
<code>
<pre>
...
class DataCheck {
    public function isValid() {
        ...
        <p class="theme__text">
            Мы написали реализацию класса DataCheck. Это сервис, который проверяет дату. Как можно использовать данный функционал в нашем приложении? <br>
            Функционал - это кастомный скрипт, который расширяет базовые возможности Laravel (код, который пишем мы под определенную задачу). <br>
...
<p class="theme__text">
    Но!<br>
    Мы привыкли: <br>
    а) Мы не создаем экземпляр класса <br>
    б) Используем статические методы <br>
    Вот так мы привыкли использовать функционал Laravel:<br>
</p>
<code>
<pre>
// SomeController.php
...
<p class="theme__text">
    Пункт "а" можно реализовать с помощью сервис-контейнера, в котором будет хранится экземпляр класса DataCheck, об этом позаботится сервис-провайдер. <br>
    Пункт "б" можно реализовать с помощью фасада, который предоставляет статические интерфейс к объектам классов, зарегистрированных в сервис-контейнере. <br>
    Хорошо. Как это реализовать?
    <br><br>

    2) Познакомися с сервис-контейнером.<br>
    Сервис-контейнер - это хранилище объектов классов, которые вам доступны при загрузке. Сервис-контейнер хранилище функционала, который есть у вас в приложение. Приложение - это и есть сервис-контейнер.
    <br><br>

    То есть сервис контейнер - это и есть само приложение, в нем хранятся объекты классов, которые и предоставляют функционал для приложения. В объекте сервис-контейнера, который представляет собой приложение хранятся все загруженные сервисы, то есть весь функционал: объекты классов сервисов, которые доступны для вас.
    <br><br>

    Грубо говоря, сервис-контейнер - объект, в свойствах которого содержаться объекты других классов, в которых содержится некий функционал и к которому вы можете обращаться.
    <br><br>

    Хорошо. Каким образом этот функционал попадает в сервис-контейнер? Как его туда добавить?
    <br><br>

    3) Создание пройвайдера.<br>
    Сервис-провайдер - поставщик услуг, с помощью которого реализуется связь между каким-то функционалом и сервис-контейнером. Он не содержит в себе функционал, он связывает некий функционал (обычно это ваш кастомный класс, как например DataCheck) с сервис-контейнером. <br>
    Можно скзаать что он добавляет функционал в сервис-контейнер/приложение.
    <br><br>

    В каталоге config/app.php в поле 'providers', содержится список тех сервис-провайдеров, которые по-умолчанию загружаются с загрузкой фреймворка. Каждый из этих сервис-провайдеров добавляет функционал в сервис-контейнер/приложение, добавляет определенные объекты в сервис-контейнер, которые содекржат необходимый функционал.
    <br><br>

    Создадим провайдер вручную: создадим файл DataCheckServiceProvider в App\Providers.
</p>
<code>
<pre>
...
<p class="theme__text">
    Теперь DataCheckServiceProvider будет создавать экземпляр класса DateCheck и добавлять его в сервис-контейнер. <br>
    Также, чтобы это работало, нужно в массиве 'providerd' указать данный сервис-провайдер, так как по-умолчанию он не выполняется. <br><br>

    4) Добавляем сервис-провайдер в загрузку приложения<br>
    Сервис-провайдер нужно загрузить сразу при загрузке framework, то добавте его в config/app.php в массив providers.
</p>
<code>
<pre>
'providers' => [
...
<p class="theme__text">
    ...
    Я так понимаю мы создали сервис-проаайцдер для нашщего функционала, в нем написали, чтобы он возвращает экземпляр класса, далее зарегали его. Теперь нужно сделать чтобы методы были статическими.

    <br><br>
    Хорошо, теперь в нашем сервис-контейнере есть экземпляр нашего класса DataCheck. Теперь сделаем, чтобы функционал этого класса был доступен так, как будто методы статические. В этом нам поможет фасад
    <br><br>

    4) Познакомися с Фасад.
...
<p>
    Насчет именований:
    1) Название файла сервиса должно совпадать с названием класса.
    // app/Services/DataCheck
    namespace App\Services;
    class DataCheckk {
        public function isValid() {
            return 'work';
    ...
    2) Название фасада файла фасада должно совпадать с названием класса определенного в фасаде.
    // app/Facades/test
    namespace app\Facades;
    ...
    3) К првоайдеру есть определнный стиль именования:
    название + ServiceProvider
    // app/Providers/DataCheckServiceProvider
    namespace App\Providers;
    use Illuminate\Support\ServiceProvider;
    class DataCheckServiceProvider extends ServiceProvider {
        // Привязываем кастомные (пользовательские) классы, которые содержат функционал к сервис-контейнеру.
    ...
    4) Первый параметр, который мы указываем в bind() при регистрации сервиса в првоайдере, мы указываем название ячейки в сервис-првоайдере, поэтому оно дложно совпадать с фасадом.
...
Ячейка test значит и getFacadeAccessor возвращает test
5) Ну и последнее.
В конфиге в массиве priveerds
App\Providers\DataCheckServiceProvider::class, - указываем название провайдера
И для фасада в массиве allies
'DataService' => App\Facades\test::class - указываем то как называется файл и соотвествено класс.

Теперь используя в контрлоерах
use DataService;
DataService::isValid() - можно статически получить доступ.
</p>

<p>
Вот весь код:
1) Кастомный сервис
// app\Services\DataCheckk
namespace App\Services;
class DataCheckk {
    public function isValid() {
        return 'work';
...
3) Регистрация провайдера в загрузчиках
config/app массив providers
App\Providers\DataCheckServiceProvider::class,
4) Создание Фасада
// app/Facades\test
namespace app\Facades;
...
class test extends Facade {
    protected static function getFacadeAccessor() {
        // тут коменды нужны, типо test так как в методе bind в сервис провайдере так называется ячейка. этот метод прсото возвращает имя ячейки в сервис контейнере нужного нам экземпляра определенного класса
        return 'test';
    }
}
5) Регистрация фасада
config/app  массив allies
...
{{-- ===================== --}}
    <div class="theme">
        ...
        <p class="theme__text">
            ...
            Точкой входа для всех запросов к приложению Laravel является файл public/index.php - это отправная точка для загрузки остальной части фреймворка. Все запросы направляются в этот файл настройками вашего веб-сервера (Apache/nginx).<br>
            Файл index.php загружает автозагрузчик, который сгенерировал Composer, а затем извлекает экземпляр Laravel-приложения из скрипта bootstrap/app.php.
            <br><br>
            ...
            Одно из наиболее важных действий при загрузке ядра - это загрузка сервис-провайдеров для вашего приложения. Сервис-провайдеры — поистине ключ к инициализации и настройке Laravel-приложения, так как они отвечают за начальную загрузку всевозможных компонентов фреймворка: таких как БД, очередь, проверка ввода и маршрутизация. Сервис-провайдеры — важнейший элемент всего процесса начальной загрузки Laravel.Они отвечают за загрузку и настройку всех возможностей фреймворка. <br>
            ...
            Далее Request поступает в маршрут для обработки. Маршрут отправит запрос по пути, а также запустит посредника, соответствующего роуту.
            <br><br>

            Итог (кратко):<br>
            1) Создаётся экземпляр приложения<br>
            2) Регистрируются сервис-провайдеры<br>
            3) Приложение загружено (создан сервис-контейнер)<br>
            4) Запрос передается в загруженное приложение.
    </div>
...
<div class="theme">
    ...
    <p class="theme__text">
        Сервис-провайдеры - центральное место для конфигурирования вашего приложения - они лежат в основе первоначальной загрузки всех приложений на Laravel.
        <br>
        Но что мы понимаем под "первоначальной загрузкой"? В общих чертах, мы имеем ввиду регистрацию таких вещей, как биндингов в IoC-контейнер (фасадов и т.д.), слушателей событий, фильтров роутов и даже самих роутов.
        <br><br>

        Сервис-провайдеры вашего приложения по-умолчанию хранятся в директории app/Providers.
        <br><br>

        Все сервис-провайдеры настраиваются в конфиге config/app.php массива providers. В массие providers перечислены все классы сервис-провайдеров, которые загружаются для вашего приложения. Конечно, многие из них являются "отложенными" провайдерами, т.е. они не загружаются при каждом запросе, а только при необходимости.
        <br><br>

        Если вы хотите расширить базовый функционал - то вы должны написать сначала его, потом создать для него сервис-провайдер и зарегистрировать в своем приложение или в AppServiceProvider.
        <br>
        AppServiceProvider по-умолчанию почти пустой. Этот провайдер является отличным местом для добавления в ваше приложение собственной начальной загрузки и привязок сервис-контейнера. Конечно, в случае больших приложений, возможно вы захотите создать несколько файлов сервис-провайдеров, каждый со своей специализацией.
        <br><br>
        ...
        <br>
        Сначала будет вызван метод register для всех сервис-провайдеров, а когда все они будут зарегистрированы, будет вызван метод boot.
        <br><br>
    </p>
...
<h3 class="theme__subtitle">
    ...
    <p class="theme__text">
    Ваши привязки сервис-контейнера будут зарегистрированы в сервис-провайдерах. <br>
    Если классы не зависят от каких-либо интерфейсов, то нет необходимости связывать их в контейнере. <br>
    Не нужно объяснять контейнеру, как создавать эти объекты, поскольку он автоматически извлекает такие объекты при помощи рефлексии.
    </p>
    <h4 class="theme_subtitlex2">
    Простые связывания
    </h4>
    <p class="theme__text">
    В сервис-провайдере всегда есть доступ к контейнеру через свойство $this->app. <br>
    Зарегистрировать привязку можно методом bind, передав имя того класса или интерфейса, который мы хотим зарегистрировать, вместе с функцией-замыкания Closure, которая возвращает экземпляр класса:
    </p>
    <code>
    <pre>
